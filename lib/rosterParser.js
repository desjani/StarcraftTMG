...
export function parseRoster(flat, options = {}) {
  ...
  const tacticalCardDetails = tacticalCardIds.map(id => {
    ...
    return {
      ...
      slots: hit?.slots ?? {},
      faction: typeof hit?.faction === 'string' ? hit.faction : '',
      tags: typeof hit?.tags === 'string' ? hit.tags : '',
      frontUrl: typeof hit?.frontUrl === 'string' ? hit.frontUrl : '',
      isUnique: Boolean(hit?.isUnique),
      resource: typeof hit?.resource === 'number' ? hit.resource : null,
      gasCost: typeof hit?.gasCost === 'number' ? hit.gasCost : null,
      abilities: Array.isArray(hit?.abilities) ? hit.abilities : [],
    };
  });
