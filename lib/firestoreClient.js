  ...
  const results = await Promise.all(
    ids.map(async (id) => {
      const tryReverseId = (value) => {
        ...
        return reversed && reversed !== value ? reversed : null;
      };

      try {
        let doc;
        try {
          doc = await fetchDocument('tactical_cards', id);
        } catch {
          const reversedId = tryReverseId(id);
          if (!reversedId) throw new Error('tactical-card-not-found');
          doc = await fetchDocument('tactical_cards', reversedId);
        }
        const boosts = Array.isArray(doc.boosts) ? doc.boosts : [];
        const abilities = boosts
          .map((boost) => {
        ...
        return {
          ...
          name: doc.name ?? id,
          slots: doc.slots ?? {},
          faction: typeof doc.faction === 'string' ? doc.faction : '',
          tags: typeof doc.tags === 'string' ? doc.tags : '',
          frontUrl: typeof doc.frontUrl === 'string' ? doc.frontUrl : '',
          isUnique: Boolean(doc.isUnique),
