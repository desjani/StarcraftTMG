 ...
 */
export async function deleteRoster(seed, { idToken } = {}) {
  const normalizedSeed = normalizeSeedId(seed);
  const baseUrl = getBaseUrlForSeed(normalizedSeed);
  const url = `${baseUrl}/shared_rosters/${encodeURIComponent(normalizedSeed)}?key=${FIREBASE_API_KEY}`;
...
function getBaseUrlForSeed(seed) {
  const normalizedSeed = normalizeSeedId(seed);
  if (/^[A-Z0-9]{6}$/.test(normalizedSeed)) return OFFICIAL_BASE_URL;
  if (/^[A-Z0-9]{7}$/.test(normalizedSeed)) return USER_BASE_URL;
...
export async function fetchDocument(collection, id) {
  const normalizedId = normalizeSeedId(id);
  const baseUrl = getBaseUrlForSeed(normalizedId);
  const url = `${baseUrl}/${collection}/${encodeURIComponent(normalizedId)}?key=${FIREBASE_API_KEY}`;
  const res = await fetch(url);
  if (!res.ok) {
    const body = await res.json().catch(() => ({}));
    const msg = body?.error?.message ?? `HTTP ${res.status}`;
